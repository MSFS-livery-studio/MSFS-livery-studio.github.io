...
async function connectGoogle() {
  try {
    ...
    console.error(err);
    setStatus(`Google connection failed: ${err.message}`);
...
function closeGoogleSetup() { ui.googleSetupModal.classList.add('hidden'); }
...
async function restoreProject(project) {
  ...
  ui.mirror.checked=!!project.mirror; ui.guideToggle.checked=project.showGuide !== false;
...
async function findCloudProjectFile() {
  ...
  const r = await driveRequest(`https://www.googleapis.com/drive/v3/files?spaces=appDataFolder&q=${q}&fields=${fields}&orderBy=modifiedTime%20desc`);
...
async function saveCloudProject({silent=false}={}) {
  ...
  cloudSaving=true;
  if(!silent){ setStatus('Saving project to Google Drive…'); ui.btnCloudSave.disabled=true; }
  try {
    ...
    if (!file) {
      const create = await driveRequest('https://www.googleapis.com/drive/v3/files?fields=id,name', {
        ...
        body:JSON.stringify({name:DRIVE_PROJECT_NAME, parents:['appDataFolder'], mimeType:'application/json'})
      });
...
async function loadCloudProject() {
  ...
  if (!file) { toast('No cloud project found'); setStatus('No cloud project found'); return; }
...
function scheduleCloudAutosave() {
  ...
  cloudSaveTimer=setTimeout(async()=>{
    ...
    try { await saveCloudProject({silent:true}); }
    catch(err){ console.error(err); setStatus(`Cloud autosave failed: ${err.message}`); }
  }, 10000);
}
...
async function init() {
  ...
  try {
    ...
    if (configuredGoogleClientId()) {
      ui.btnGoogle.textContent='Connect Google';
      ui.btnGoogleTop.textContent='Google';
    }
    setStatus('Ready');
...
ui.btnCloudSave.addEventListener('click', async()=>{
  try { await saveCloudProject(); } catch(err){ console.error(err); setStatus(`Cloud save failed: ${err.message}`); toast('Cloud save failed'); }
});
ui.btnCloudLoad.addEventListener('click', async()=>{
  try { await loadCloudProject(); } catch(err){ console.error(err); setStatus(`Cloud load failed: ${err.message}`); toast('Cloud load failed'); }
});
...
$('btnGoogleSetupSave').addEventListener('click', async()=>{
  ...
  googleTokenClient=null; closeGoogleSetup();
  await connectGoogle();
});
