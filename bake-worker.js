/* MSFS Livery Studio Web v0.7.3 - background UV baker */
...
async function bakeOne(profile,profileBase,surfaceData,ti){
  ...
  return {file:out.file,data:await encodePNG(dst)};
}
onmessage=async ev=>{
  if(!['export','preview','preview-all'].includes(ev.data?.type))return;
  try{
    const {profile,profileBase,surfaceBuffers,fields,version,index}=ev.data;
    ...
    if(ev.data.type==='preview'){
      const ti=Math.max(0,Math.min(profile.texture_outputs.length-1,Number(index)||0));
      const texture=await bakeOne(profile,profileBase,surfaceData,ti);
      postMessage({type:'preview-done',buffer:texture.data.buffer,file:texture.file},[texture.data.buffer]);
      return;
    }
    if(ev.data.type==='preview-all'){
      const results=[];
      const transfer=[];
      const mapped=profile.texture_outputs
        .map((t,i)=>({t,i}))
        .filter(x=>x.t.mapped_in_surface_editor!==false);
      for(let n=0;n<mapped.length;n++){
        const {t,i}=mapped[n];
        progress(35+Math.round((n+1)/mapped.length*60),`Baking 3D texture ${n+1}/${mapped.length} · ${t.material}`);
        const texture=await bakeOne(profile,profileBase,surfaceData,i);
        results.push({index:i,file:texture.file,buffer:texture.data.buffer});
        transfer.push(texture.data.buffer);
      }
      postMessage({type:'preview-all-done',textures:results},transfer);
      return;
    }
    const textures=[];
    for(let ti=0;ti<profile.texture_outputs.length;ti++){
      ...
      textures.push(await bakeOne(profile,profileBase,surfaceData,ti));
    }
